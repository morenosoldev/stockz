 ...
 */
export function useScanLogs({ runId, enabled = true, onComplete, onError }: UseScanLogsOptions) {
  ...
  useEffect(() => {
    ...
    const url = `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/v1/scan/logs/${runId}`;
    ...
    eventSource.addEventListener('stats', (event) => {
      try {
        ...
        console.error('[SSE] Failed to parse stats message:', event.data, error);
      }
    });
    ...
    eventSource.addEventListener('complete', (event) => {
      try {
        ...
        console.error('[SSE] Failed to parse complete message:', event.data, error);
      }
    });
